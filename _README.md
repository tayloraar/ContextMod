...
What the mod can do is count, and then move the work cleanly when you say so. Two pieces work together. The `Turn` field in `_STRATEGY_STATE.md > Current` increments on every Tier 2 flush. Set a threshold in `_INDEX.md > Configuration > Turn warning threshold` and the protocol surfaces a soft prompt when the count exceeds it. The prompt suggests running `SAVE` to checkpoint the substrate, then `EXPORT` to capture the recent conversation, then opening a fresh chat pointed at the same files. The substrate carries the facts; the export carries the texture. The new chat reads both and picks up where you left off.
...
## Coverage tracking and deep reads

The protocol tracks what the model has actually read versus what it has only sampled. This is what stops the model from confidently producing output about material it never properly ingested — a failure mode the original protocol design didn't fully address.

How it works at a user level:

The `_INDEX.md` carries a read-state tracker that records coverage state for substrate-grade source material. States are `[skim]`, `[partial: §X]`, or `[full]`. When you install the mod over an existing project (multiple novels, a code repository, accumulated chat logs, research papers), most of that material starts as `[skim]` because the model hasn't actually read it yet. The model knows this and operates honestly: when work touches material that's still `[skim]` or `[partial]`, the model stops and asks before producing output.

The surfacing prompt is terse:

> *Pending: I have only `[skim]` coverage on [source]. Should I deep-read first, or proceed with partial knowledge?*

You decide. *Deep-read now* triggers the chunked-read procedure. *Proceed partial* tells the model to operate on what it has and surface uncertainty when relevant. *Defer* postpones the question. *Different action* means you want to do something else entirely.

The chunked-read procedure ingests source material in passes. Default chunk size is whichever is *smaller* of a natural boundary (chapter, section, file) or 8,000 words. The model reads pass by pass, distils canonical entries into `_KNOWLEDGE.md` after each pass, updates the read-state tracker, and continues automatically. Once authorised, the read auto-continues through all passes. You can interrupt at any pass boundary by saying *STOP* — the substrate stays accurate about what was read, and unread material remains marked `[skim]` for next time.

This generalises beyond initial corpus ingestion. The same procedure handles bringing exported chat logs into a new project, adding a new code repository to your working substrate, or layering research papers in over time. The read-state tracker accumulates honest coverage as the project matures.

Source material in non-markdown formats (`.docx`, `.pdf`, `.epub`) needs converting to markdown chunks before ingestion. For long documents, a typical convention is `Chapter_01.md`, `Chapter_02.md`, etc. Most platforms can do this conversion with a one-line command (Pandoc handles most cases: `pandoc novel.docx -o novel.md` then split). The conversion is one-time preparation; after that the protocol operates on the chunks.

When it matters most: the read-state tracker is most valuable when you install the mod over an existing project that has substantial material. On a blank-slate project where you're building the substrate alongside the work, coverage tracking matters less because everything is being ingested as it's created. On an existing-project install, coverage tracking is what prevents the model from confidently producing output on material it has only seen the metadata of.
...
**Notes:**
- Gemini's instruction adherence on multi-step protocols varies more than Claude/ChatGPT. Run a few sessions and watch whether the Tier 2 checklist actually fires.
...
**Upgrading the protocol mid-session.** When swapping to a newer version of the protocol on an in-flight session, the cleanest pattern is `UNINSTALL` followed by a fresh session that installs the new version. Reason: even after `UNINSTALL` removes the files, the previous protocol's rules remain in the current session's conversation history. The model has already read them. Installing a new version on top of that context produces rule conflicts the model resolves unpredictably. A fresh session reads the new rules cleanly without the old rules competing for adherence. The substrate (`_KNOWLEDGE.md`, `_INDEX.md`, `_STRATEGY_STATE.md`) survives the session change and can be loaded by the new chat without losing project state. On Cowork and Claude Code, this is opening a new task. On Claude.ai with Projects, this is starting a new chat within the same project. On platforms with weaker session boundaries (one-off Claude.ai chats, ChatGPT one-offs), the swap-in-place may work but is less reliable; new session is the safer path.
...
- Models vary in how reliably they follow the Tier 2 mandatory checklist. Compliance is itself a variable; observe it before drawing conclusions about the protocol.
