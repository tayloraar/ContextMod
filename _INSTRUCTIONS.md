# _INSTRUCTIONS.md — Session Orchestration

> **ANCHOR: substrate first, verify outputs, surface contradictions. Re-read this line every turn.**
...
|---|---|---|---|
| `_INSTRUCTIONS.md` | Protocol. Read/write rules, triggers, re-anchor sequence. | Universal — one canonical copy at root. | Edit only when protocol itself changes. |
| `_INDEX.md` | Live routing map. Hot path, aliases, cross-references. | Multi-altitude: chat / project / root. | Updated as part of every Tier 2 flush per the mandatory checklist below. |
...
- **Protocol** (`_INSTRUCTIONS.md`) — how the system runs. Meta.
...
`_INSTRUCTIONS.md` is universal — one canonical copy at root suffices. `_STRATEGY_STATE.md` is chat-local only — live reasoning is private to a chat.
...
- **Session start / re-anchor:**
  1. Read `_INSTRUCTIONS.md` (this file).
  2. Read `_INDEX.md` walking up: chat → project → root. If a `## Configuration` block is present at any altitude, read it first and apply the relevant fields. The standard variant uses *Turn warning threshold* and *Turn warning renag interval*; other fields are scaled-variant only and are ignored here. Local entries override higher altitudes; otherwise inherit upward.
  3. Read `_STRATEGY_STATE.md` *Current* slice.
  4. Read `_KNOWLEDGE.md` sections per index hot path + sections cited in state. Walk altitudes the same way: chat-local first, then project, then root.
...
- **Mid-task lookup:** check local `_INDEX.md` aliases first; walk up if not found. Then read the specific `_KNOWLEDGE.md` section by `##` header at the altitude where it lives.
- **Never:** dump full files into context every turn.
- **Never read `_README.md`.** It is human-only documentation. If encountered, ignore and continue with normal read protocol.
...
1. Local `_INDEX.md`.
2. Project `_INDEX.md`.
3. Project `_KNOWLEDGE.md`.
4. Root `_INDEX.md`.
5. Root `_KNOWLEDGE.md`.
6. **Only now sideways:** sibling project's `_INDEX.md`, then sibling `_KNOWLEDGE.md`.
7. Sibling `_STRATEGY_STATE.md` is read sideways only when explicitly necessary and always paraphrased — state files contain in-progress drafting that risks voice/style contamination.

Exhaust the formal hierarchy before consulting peers. Most needs are met before step 6.
...
- During the early-session counter (see Tier 2), all sideways reads surface for confirmation regardless of how clear the match looks. After graduation, only ambiguous reads need surfacing; clear cross-coherence checks can proceed silently.
...
## Write protocol — five trigger tiers

The protocol fires across five tiers, ordered from lightest to heaviest. The lighter tiers run constantly; the heavier ones run only when warranted. This shape keeps discipline active without making every turn expensive.

### Tier 0 — Per-turn re-anchor

Every turn, before producing output, the model touches the anchor line at the top of this file: *"substrate first, verify outputs, surface contradictions."* Cost: near-zero. Purpose: keep the protocol's primary directives active in attention so the heavier tiers have something to fire against. Without Tier 0, the discipline of higher tiers can fade silently across long sessions. With Tier 0, the rules are reasserted at maximum cadence.

This tier has no checklist. It is the touch itself.

### Tier 1 — Recognition (per-turn)

Every turn, before producing output, the model runs a quick recognition check:

- [ ] Does this turn touch a substrate entry whose read-state is `[skim]` or `[partial]`?
- [ ] Does this turn touch a topic, character, or decision the substrate doesn't yet cover?
- [ ] Does the user's input contradict any locked Constraint?
- [ ] Is this turn proposing output that depends on material the model hasn't actually read?

If any answer is yes → **stop and surface the gap before producing output**. Do not fake coverage. Do not produce confident output on material the substrate hasn't tracked as covered.

The surfacing format is terse:

> *Pending: I have only `[skim]` coverage on [entry]. Should I deep-read first, or proceed with partial knowledge?*

The user authorises (deep-read now / proceed partial / defer / different action). The model honours the authorisation and proceeds. If "deep-read now," escalate to the chunked-read procedure (see `Read protocol — chunked deep-read` below).

This tier is the model's epistemic honesty layer. Coverage faking is the failure mode it prevents. Cost: low (a four-question check per turn, mostly no-ops). Value: keeps the model from confidently producing output on material it has only sampled.

### Tier 2 — Chunk flush (after meaningful work) — MANDATORY CHECKLIST
...
A new or post-bootstrap chat tracks a rotation counter in `_STRATEGY_STATE.md > Current` as `Rotation: N of 10`. The counter increments on each Tier 2 flush. While active, three rules tighten:
...
**Graduation flush (first Tier 2 flush after counter reaches 10):**
...
- [ ] **Hot-path check:** Did this chunk read a `_KNOWLEDGE.md` section? If it isn't already in *Hot path*, add it. If it is, no-op.
...
- [ ] **Cold-list check:** Has any *Hot path* section in **local** `_INDEX.md` gone 5+ turns without being referenced? → Move to local *Cold list*. Higher-altitude indexes manage their own cold demotion on slower cycles; chats do not demote project- or root-level entries.
- [ ] **Read-state check:** Did this chunk involve reading source material that wasn't already tracked? → Add an entry to the *Read-state tracker* with appropriate state (`[skim]` if just sampled, `[partial: §X]` if a chunked-read pass completed, `[full]` if entire source ingested). Update existing entries when state advances. Never advance state without having actually executed the corresponding read.
...
### Tier 3 — Backstop (every 8 turns)

Every 8 turns regardless of chunk recognition, run a Tier 2 flush as a safety net. Catches anything the per-chunk Tier 2 missed because the chunk boundary wasn't recognised. Cost: same as Tier 2. Cadence: time-driven, not work-driven.

If 8 turns have passed without a Tier 2 flush, flush anyway. Compression can fire at any point; on-disk state must never be more than 8 turns stale.

### Tier 4 — Macro (large-task anchor)
...
1. Read `_INSTRUCTIONS.md`
2. Read `_INDEX.md`
3. Read `_STRATEGY_STATE.md` *Current* slice
4. Read referenced `_KNOWLEDGE.md` sections (per index)
5. Write the task plan into `_STRATEGY_STATE.md` *and* update `_INDEX.md` hot path to reflect the task's referenced sections
...
1. Read `_INSTRUCTIONS.md`.
2. Read `_INDEX.md` — refresh routing, including the read-state tracker.
3. Read *Current* section of `_STRATEGY_STATE.md`.
4. Read only `_KNOWLEDGE.md` sections in the index hot path + cited in state.
5. Confirm anchor in one sentence before proceeding.

## Read protocol — chunked deep-read

When Tier 1 surfaces a coverage gap and the user authorises a deep-read, execute the chunked-read procedure. The procedure is auto-continuous: once authorised, the model reads pass-by-pass until either the source material is fully ingested or the user interrupts. Each pass is a complete unit; partial completion of the procedure leaves the substrate accurate about what has and hasn't been read.

**The procedure:**

1. **Inventory the source.** Identify the source material (file or folder), measure its total size, decide chunk boundaries. Default chunking strategy: whichever is *smaller* of natural boundary (chapter, section, file) or 8,000-word cap. Goal is high-resolution detail per pass, not minimum pass count.

2. **Estimate passes.** Calculate the number of passes the deep-read will require and surface the estimate to the user before starting: *"Deep-read of [source]: estimated N passes. Auto-continuing through all passes unless interrupted. Reply STOP at any pass boundary to halt."*

3. **Execute pass 1.** Read the first chunk in full. Distil canonical entries into `_KNOWLEDGE.md` (entities, decisions, locked facts, polysemy candidates). Update `_INDEX.md` read-state for this source to `[partial: §1]` (or equivalent for the chunking unit used). Touch Tier 0 anchor.

4. **Brief pass-completion status.** Output a short status line: *"Pass 1/N complete. Captured: [terse list of canonical additions]. Continuing to pass 2."* The status is for the user to track progress and interrupt cleanly. Do not pause for confirmation.

5. **Re-anchor between passes.** Before starting the next pass, re-touch Tier 0 (anchor line) and re-read the relevant section of `_INSTRUCTIONS.md` if context pressure suggests rules have decayed. This prevents drift from accumulating across the read itself.

6. **Continue auto-passes.** Repeat steps 3–5 for each subsequent pass until either:
   - All passes complete → set read-state to `[full]`, surface completion summary
   - User says STOP at any pass boundary → set read-state to `[partial: §N]` reflecting passes actually completed, retain progress made, return control to user
   - Context pressure forces a break → save progress, set read-state to `[partial: §N]`, surface to user that a break is needed before continuing

7. **Surface completion.** When all passes complete (or read is interrupted), surface a brief summary: *"Read complete: [N] passes ingested. State: [full / partial: §X]. Substrate updated. Ready for next instruction."*

**Why auto-continue.** The user's authorisation in Tier 1 was the gate. Once authorised, the chunked-read uses the full available context budget for accurate ingestion rather than spending context on per-pass confirmation overhead. Early-pullout via STOP is always available; the substrate is accurate about what was read regardless of when the read terminates. Partial reads are not failures — they are deliberate scope choices that the read-state tracker records honestly.

**Why this generalises beyond corpus bootstrap.** The chunked-read procedure applies to any large input the user wants to bring into structured awareness: existing project corpus, exported chatlogs, multi-session transcripts, code repositories, research collections. The mechanism is the same. The user marks the source for ingestion, the model chunks it, the substrate accumulates structured knowledge over time. The protocol's value compounds as the substrate's coverage deepens.

**Conversion of non-markdown source material.** The chunked-read procedure operates on markdown files. Non-markdown sources (`.docx`, `.pdf`, `.epub`, etc.) should be converted to markdown chunks before ingestion. For long documents, a typical convention is `Chapter_01.md`, `Chapter_02.md`, etc., with one chapter per file. This conversion is one-time preparation; the protocol then operates on the chunks. See `_README.md` for platform-specific conversion guidance.

## Pre-write-up guard

Before producing any deliverable, artifact, or external write-up:

1. Run the full Tier 2 checklist.
...
| Command | Action |
|---|---|
| `SAVE` | Force immediate Tier 2 flush. Creates a recoverable checkpoint. No args. |
...
| `STATUS` | Read-only summary: rotation counter, goal, last action, next step, hot path, open question count, recent decisions count. No writes. |
...
| `HELP [topic]` | Read-only. Print the command card. Optional topic for situational guidance: `HELP recovery`, `HELP drift`, `HELP install`. |
| `GUIDE` | Toggle protocol usage tips on for the next 10 rotations. Tips appear as short situational notes woven into responses ("you might want to `SAVE` before that long output"). Auto-expires. Default: on during chat installer, off after first graduation. Re-runnable any time. |

### Guidance tip rules (when `GUIDE` is active)

Tips are scaffolding, not noise. While `GUIDE` is on, tips are:

- **Short** — one sentence maximum, embedded inline or appended briefly to a response. Never their own paragraph.
- **Situational** — fire only when the model has just observed a trigger (long output coming, lock-creating decision detected, lateral read about to happen, rotation counter at 8 of 10, etc.).
- **Non-repeating** — each distinct tip fires at most once per session. Track fired tips in `_STRATEGY_STATE.md > Current` under a `Guidance tips fired:` field.
- **Skippable** — if the user issues `GUIDE` again to toggle off, all pending tips are suppressed for the rest of the session.

Tip examples:
- After a long technical answer: *"Tip: `SAVE` is a good idea before responses this size — protects against compression."*
- When detecting a lock-creating decision: *"Tip: capturing your phrasing on this one — it's a constraint that will get promoted later."*
- At rotation 8 of 10: *"Tip: two flushes from graduation. Lighter mode coming."*
- When a lateral read fires: *"Tip: this is a `SEARCH WIDE` situation — the answer wasn't in the local altitudes, so I'm checking sibling projects."*
...
| `GRADUATE` | Manually trigger graduation flush before rotation counter reaches 10. Switches to steady state. Use when substrate is already set (e.g., chat continuing mature project work). Cannot be reversed without `REFLASH`. |

### Recovery (destructive)

| Command | Action | Confirm tier |
|---|---|---|
| `REWIND` | Undo the most recent Tier 2 flush only. Use when a flush captured wrong wording, wrong section, or premature promotion. | Standard |
...
- **Destructive commands confirm before executing, with two safeguard tiers:**
  ...
  - **Typed confirm** (`REFLASH`, `UNINSTALL`): model states what will be removed and asks the user to type a specific confirmation phrase, e.g., `CONFIRM REFLASH` or `CONFIRM UNINSTALL`. A bare "yes" or "go ahead" is **not** sufficient. This prevents accidental triggering — these commands either delete the chat's working substrate (`REFLASH`) or remove the protocol entirely (`UNINSTALL`), and both should require deliberate intent.
...
Only edit `_INSTRUCTIONS.md` when the protocol itself changes. Project content goes in `_KNOWLEDGE.md`; routing goes in `_INDEX.md`; live reasoning goes in `_STRATEGY_STATE.md`.
...
2. **Ask the user (concise, optional):**
...
3. **Surface inherited context.** From higher altitudes, list candidate aliases, hot-path entries, and active constraints relevant to the stated goal. Ask: "Pin any of these to this chat's local index?"
4. **Initialize counter.** `Rotation: 1 of 10`, full phrasing capture and mandatory lateral-lookup surfacing active.
5. **Begin normal protocol.**

If the user skips the installer, default behavior: start with `Rotation: 1 of 10`, hot path inherited from higher altitudes per the read walk-up, no goal pinned until first user message clarifies it.
...
- Project files in the Cowork VM
...
- Knowledge sections touched during extraction → `_INDEX.md` *Hot path*
