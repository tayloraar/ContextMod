...
| `_INDEX.md` | Live routing map. Configuration block, hot path, aliases, cross-references. | Multi-altitude: chat / project / root. | Hot path updates automatically on retrieval; other sections updated per Tier 2 flush. |
...
2. At each Tier 2 flush, check pins. If a pin's last-retrieved count is older than the decay window, the pin has decayed.
...
1. Read `_INSTRUCTIONS_SCALED.md`.
2. Read `_INDEX.md` Configuration block, Hot path, Aliases, read-state tracker. Hot path is the working set; reading it refreshes routing automatically.
...
5. **Re-anchor between passes.** Before starting the next pass, re-touch Tier 0 (anchor line) and re-read the relevant section of `_INSTRUCTIONS_SCALED.md` if context pressure suggests rules have decayed. This prevents drift from accumulating across the read itself.
...
**Hot-path interaction (scaled-only).** When a chunked-read produces canonical entries that are likely to be referenced again soon, the LRU hot path will surface them automatically without manual intervention. Pin entries that are load-bearing across the entire project but might not be retrieved frequently enough to stay hot.
